...
if __name__ == '__main__':
    ...
    # Whether to extract serialized model.
    parser.add_argument('--serialize', type=int)
    ...
    if args.serialize == 1:
        ...
        if voxel_dim == 64:
            ...
            new_weight = {
                'voxels': weight['voxels'],
                'voxel_mask': weight['voxel_mask'],
            }
            new_weight['weight0'] = torch.cat(
                [bias1[:, None], linear1],
                1,
            )
            new_weight['weight1'] = torch.cat(
                [bias4[0, None], linear4[0]],
                0,
            )
            new_weight['weight2'] = torch.cat(
                [bias4[1:, None], linear4[1:], linear42],
                1,
            )
            new_weight['weight3'] = torch.cat(
                [weight['mlp2.2.bias'][:, None], weight['mlp2.2.weight']],
                dim=-1,
            )
            ...
            # Recorder positional encoding in a way easy to be processed
            # by the cuda code.
            w2 = torch.cat(
                [
                    w21,
            ...
            params = torch.cat(
                ...
                0,
            ).contiguous()  # Padding to fit a warp in cuda.
        ...
        if voxel_dim == 32:
            features = (
                features @ new_weight['weightin'][:, 1:].T).contiguous()

        valid = (idxs < N).all(0)

        # Indices of occupancy mask is a subset of indices of feature vectors.
        is_corner = new_weight['voxel_mask'][idxs[0][valid], idxs[1][valid],
                                             idxs[2][valid]]
