...
from diver.configs.nerf_synthetic_coarse import default_options
from diver.model.diver import DIVeR
from diver.utils.dataset import BlenderDataset
from diver.utils.dataset import TanksDataset
...
def main():
    ...
    if dataset_name == 'blender':
        ...
        dataset = dataset_fn(dataset_path,
                             img_wh=hparams.im_shape[::-1],
                             split='train')
        ...
        for batch in tqdm(dataloader):
            ...
            for b_id in range(math.ceil(len(rays) * 1.0 / batch_size)):
                ...
                if hasattr(model, 'voxel_mask'):
                    ...
                    coord, mask, _ = ray_voxel_intersect(
                        xs.contiguous(), ds.contiguous(), model.xyzmin,
                        model.xyzmax, int(model.voxel_num), model.voxel_size)
                    if not mask.any():
                        continue
                    mask = mask[:, :-1] & mask[:, 1:]
                ...
                coord = torch.min((coord_in + 1e-4).long(),
                                  (coord_out + 1e-4).long())

                # check if out of boundary
                bound_mask = (
                    (coord >= args.pad) &
                    (coord <= model.voxel_num - 1 - args.pad)).all(-1)
                ...
                coord = coord[:,
                              0] + coord[:,
                                         1] * model.voxel_num + coord[:, 2] * (
                                             model.voxel_num)**2

                # scatter-max to the occupancy map
                alpha_map = torch_scatter.scatter(weight,
                                                  coord,
                                                  dim=0,
                                                  out=alpha_map,
                                                  reduce='max')

        alpha_map = alpha_map.reshape(model.voxel_num, model.voxel_num,
                                      model.voxel_num)
        torch.save(alpha_map.cpu(),
                   alpha_map_path)  # save in the model weight folder
    else:
    ...
    print(f'{voxel_mask.float().mean().item()} sapce preserved')
    ...
    if dataset_name == 'blender':
        # for nerf-synthetic, we directly store the sampled rays
        # as .npz file in the weight folder
        dataset_fn = BlenderDataset
        fine_size = [800, 800]
        dataset = dataset_fn(
            dataset_path,
        ...
        print(f'{len(fine_rays) * 1.0 / len(dataset)} rays perserved')
        ...
        for batch in tqdm(dataloader):
            ...
            # save in the mask folder inside model weight folder
            save_image(masks, mask_path / f'mask_{idx}.png')
            idx += 1
