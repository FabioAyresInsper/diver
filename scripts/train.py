# pylint: disable=missing-docstring, too-many-ancestors, no-member, invalid-name
import math
...
# from diver.configs.config import default_options
from diver.configs import nerf_synthetic_coarse as config
...
from diver.utils.dataset.fineray import FineRayDataset
...
class ModelTrainer(pl.LightningModule):
        ...
        if self.hparams.fine > 0:  # fine training
            if dataset_name == 'blender':
                ...
                dataset = TanksDataset(
                    dataset_path,
                    img_wh=(self.im_shape[1], self.im_shape[0]),
                    sample_nums=new_batch_size * 10,
                    mask_path=os.path.join(self.hparams.coarse_path, 'masks'),
                )
                return DataLoader(
                    dataset,
                    shuffle=True,
    ...
    def training_step(self, *args, **kwargs):
        ...
        # reconstruction loss
        loss_c = NF.mse_loss(rgb, rgb_h)

        # sparsity regularization loss
        loss_reg = self.hparams.l_s * self.sigma_loss(sigma)
    ...
    def validation_step(self, *args, **kwargs):
        ...
        # batch size used for rendering
        batch_size = self.hparams.batch_size * 2

        rgbs, depths = [], []
        for b_id in range(math.ceil(rays.shape[0] * 1.0 / batch_size)):
            x, d = rays[b_id * batch_size:(b_id + 1) *
                        batch_size, :3], rays[b_id * batch_size:(b_id + 1) *
                                              batch_size, 3:6]
        ...
        self.logger.experiment.add_image(
            'val/gt_image',
            rgb_h.reshape(*self.im_shape, 3).permute(2, 0, 1), batch_idx)
        self.logger.experiment.add_image(
            'val/inf_image',
            rgbs.reshape(*self.im_shape, 3).permute(2, 0, 1), batch_idx)
        self.logger.experiment.add_image(
            'val/inf_dis',
            depths.reshape(*self.im_shape,
                           1).permute(2, 0, 1).expand(3, *self.im_shape),
            batch_idx)


def add_model_specific_args(parent_parser):
    parser = ArgumentParser(parents=[parent_parser], add_help=False)
    for name, args in config.default_options.items():
        if args['type'] == bool:
            parser.add_argument(
                f'--{name}',
                type=eval,
...
def main():
    ...
    # load occupancy mask from coarse training
    if args.coarse_path is not None and last_ckpt is None:
