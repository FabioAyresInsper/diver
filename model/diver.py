...
class DIVeR(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        # Coarse model occupancy mask is different from the fine model.
        self.mask_scale = hparams.mask_scale
        ...
        if hparams.implicit:
            self.mlp_voxel = ImplicitMLP(
                hparams.implicit_network_depth,
                hparams.implicit_channels,
                hparams.implicit_skips,
                self.voxel_dim,
                hparams.implicit_point_encode,
            )
        else:
            # Randomly-initialized explicit grid.
            voxels = 1e-2 * torch.randn(
                *[self.voxel_num + 1] * 3,
                self.voxel_dim,
            )
    ...
    def init_voxels(self, evaluate=True):
        ...
        if evaluate:
            ...
            for i in tqdm(range(len(P))):
                voxel = self \
                    .mlp_voxel(P[i].to(device).reshape(-1, 3)) \
                    .reshape(N, N, self.voxel_dim)
                voxels.append(voxel)
            self.register_parameter(
                'voxels',
                nn.Parameter(torch.stack(voxels, dim=0), requires_grad=True),
            )
        else:
            self.register_parameter(
                'voxels',
                nn.Parameter(
                    torch.zeros(N, N, N, self.voxel_dim),
                    requires_grad=True,
                ),
            )
    ...
    def extract_features(self, os, ds):
        ...
        if hasattr(self,
                   'voxels'):  # check whether use explicit or implicit query
            features = integrate(self.voxels, coord_in, coord_out)
        else:
            features = integrate_mlp(self.mlp_voxel, self.voxel_num + 1,
                                     self.voxel_dim, coord_in, coord_out)
    ...
    def decode(self, coord_in, coord_out, ds, mask):
        ...
        if hasattr(self, 'voxels'):
            ...
            feature = integrate_mlp(self.mlp_voxel, self.voxel_num + 1,
                                    self.voxel_dim, coord_in, coord_out)
        ...
        x = torch.cat([x, self.view_enc(ds[torch.where(mask)[0]])], dim=-1)
    ...
    def forward(self, os, ds):
        """ find the accumulated densities and colors on the voxel grid
        given corresponding rays
        ...
        # set density and color to be zero for
        # the locations corresponding to miss hits
        sigma = torch.zeros(B, M, device=mask.device)
        sigma[mask] = sigma_
        color = torch.zeros(B, M, 3, device=mask.device)
        color[mask] = color_
